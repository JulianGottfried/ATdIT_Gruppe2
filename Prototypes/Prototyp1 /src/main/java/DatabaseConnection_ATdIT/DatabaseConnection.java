...
public class DatabaseConnection {
	...
	public static void main(String[] args) {
		...
		try (Connection con = DriverManager.getConnection("jdbc:postgresql://localhost:5432/postgres", "postgres",
			...
			Person person1 = new Person(PersonenID, Vorname, Nachname, Geburtsdatum, PLZ, Wohnort, StraßeUndHausnummer);
			...
			temp.setString(7, person1.getPersonenID());
			...
			System.out.printf("%-30.30s %-30.30s %-30.30s %-30.30s %-30.30s %-30.30s"
					+ " %-30.30s%n", "Vorname", "Nachname",
					"Geburtsdatum", "PLZ", "Wohnort", "Straße und Hausnummer", "PersonenID");

			while (rs.next()) {
				System.out.printf("%-30.30s %-30.30s %-30.30s %-30.30s %-30.30s %-30.30s %-30.30s%n", rs.getString(1),
